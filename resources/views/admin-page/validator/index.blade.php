...
<nav aria-label="breadcrumb mb-0">
    <ol class="breadcrumb breadcrumb-style2">
      ...
      <li class="breadcrumb-item">
        <a href="{{route('validator-proposal.index')}}">@yield('title')</a>
      </li>
            ...
            <div class="col-12">
                <div class="card table-responsive">
                    <div class="card-body">
                        ...
                        <!-- AKHIR TOMBOL -->
                            <table class="table table-hover table-responsive" id="table_validator">
                              <thead>
                                <tr>
                                  <th>#</th>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select class="form-select" id="diusulkan_oleh" name="diusulkan_oleh" aria-label="Default select example" style="cursor:pointer;">
                                                    ...
                                                    <option value="{{$pengusul->id}}">{{$pengusul->nama_jabatan}}</option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select class="form-select" id="diketahui_oleh" name="diketahui_oleh" aria-label="Default select example" style="cursor:pointer;">
                                                    ...
                                                    <option value="{{$diketahui->id}}">{{$diketahui->nama_jabatan}}</option>
                                                    @endforeach
                                                </select>
                                                <span class="text-danger" id="diketahuiErrorMsg" style="font-size: 10px;"></span>
                                            </div>                                           
...
<script>
    ...
    // DATATABLE
    $(document).ready(function () {
        var table = $('#table_validator').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('validator-proposal.index') }}",
            columns: [
        ...
        $("#form-tambah-edit").validate({
            submitHandler: function (form) {
                ...
                $.ajax({
                    ...
                    url: "{{ route('validator-proposal.store') }}",
                    type: "POST",
                    dataType: 'json',
                    success: function (data) {
                        ...
                        $('#table_validator').DataTable().ajax.reload(null, true);
                        Swal.fire({
                            title: 'Good job!',
                    ...
                    error: function(response) {
                        $('#pengusulErrorMsg').text(response.responseJSON.errors.diusulkan_oleh);
                        $('#diketahuiErrorMsg').text(response.responseJSON.errors.diketahui_oleh);
                        $('#tombol-simpan').html('Save');
                        Swal.fire({
                            title: 'Error!',
    ...
    $('body').on('click', '.edit-post', function () {
        ...
        $.get('validator-proposal/' + data_id + '/edit', function (data) {
            $('#modal-judul').html("Edit data");
            $('#tombol-simpan').val("edit-post");
            $('#tambah-edit-modal').modal('show');
              
            $('#id').val(data.id);
            $('#diusulkan_oleh').val(data.diusulkan_oleh);
            $('#diketahui_oleh').val(data.diketahui_oleh);
        })
    });
